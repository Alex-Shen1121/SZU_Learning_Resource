...
if __name__ == '__main__':
    ...
    max_i, max_j, max_k, max_mrr = 0, 0, 0, 0

    for i in range(10, 30, 3):
        for j in range(10, 30, 3):
            for k in range(0, 10, 2):
                weights = {'itemcf': i/10.0,
                           'binetwork': j/10.0,  'w2v': k/10.0, }
                recall_list = []
                recall_dict = {}
                ...
                recall_score = recall_final[['user_id', 'article_id', 'sim_score']]\
                    .groupby(['user_id', 'article_id'])['sim_score']\
                    .sum().reset_index()
                ...
                # 计算相关指标
                if mode == 'valid':
                    total = df_query[df_query['click_article_id']
                                     != -1].user_id.nunique()
                    ...
                    if mrr_5 > max_mrr:
                        max_i, max_j, max_k = i/10.0,  j/10.0, k/10.0
                        max_mrr = mrr_5
                        print('当前为最大值:')
                        print(max_i, max_j, max_k, max_mrr)
