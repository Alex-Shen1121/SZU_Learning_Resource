...
def train_model(df_feature, df_query):
    ...
    model = cbt.CatBoostClassifier(max_depth=16,
                                   learning_rate=0.08,
                                   n_estimators=10000,
                                   subsample=0.75,
                                   random_state=seed,
    ...
    for fold_id, (trn_idx, val_idx) in enumerate(
        ...
        joblib.dump(model, f'../user_data/model/cbt{fold_id}.pkl')
    ...
    df_sub.to_csv(f'../prediction_result/result_cbt_' +
...
def online_predict(df_test):
    ...
    for fold_id in tqdm(range(5)):
        model = joblib.load(f'../user_data/model/cbt{fold_id}.pkl')
    ...
    df_sub.sort_values(['user_id'], inplace=True)
    os.makedirs('../prediction_result', exist_ok=True)
    df_sub.to_csv(f'../prediction_result/result_cbt.csv', index=False)
